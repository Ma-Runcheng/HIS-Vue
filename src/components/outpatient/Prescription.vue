	...
	<div>
		...
		<el-table :data="prescription" @selection-change="handleSelectionChange" style="width: 100%">
			...
			<el-table-column prop="creationTime" label="创建时间">
				<template slot-scope="scope">
					<el-date-picker v-model="scope.row.creationTime" type="datetime" placeholder="选择日期时间" value-format="yyyy-MM-dd HH:mm:ss">
					</el-date-picker>
				</template> -->
			</el-table-column>
		</el-table>
		...
		<el-dialog title="新增处方" :visible.sync="dialogVisible" width="50%">
			...
			<el-table :data="druginfo" style="width: 100%%">
				<el-table-column type="selection" width="55">
				</el-table-column>
			...
			addPrescription() {
				...
				this.prescription.forEach(item => {
					item.registerId = this.$store.state.patientinfo.id;
						this.axios.post('http://localhost:8080/prescription/insertPrescription', item).then(res => {
							if (res.data == true) {
								++num;
							}
						})
				});
				if (num == this.prescription.length) {
					this.$message({
						showClose: true,
						message: '开设处方成功',
						type: 'success'
					});
					this.axios.post('http://localhost:8080/register/updateVisitState?vs=3&id=' + this.$store.state
						.patientinfo.id)
				} else {
					this.$message({
						showClose: true,
